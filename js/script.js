function handleResize() {
	...
	if (slickSlider) {
		...
		const sliderItemCard = slickSliderCard.querySelectorAll('.slick-slider-card__item');
...
function handleClickDot(element) {
	let carouselItems = document.querySelectorAll('.carousel__item');
	let carouselDots = document.querySelector('.carousel__control--dots').childNodes;
	if (!Number.isInteger(element)) {
		...
		if (index === 0 && i < 0) {
			carouselDots[carouselDots.length + i].style.backgroundColor = 'white';
